...
class PricingPlanController extends Controller
{
    ...
    {
        $telegramAddonIsInstalled = addonIsInstalled('tele-wpp');
        ...
        $request->validate([
            ...
            'telegram_bot_limit' => $telegramAddonIsInstalled ? 'required|integer|gte:-1' : 'nullable|integer|gte:-1',
        ]);
        ...
        $pricingPlan->flow_limit          = $request->flow_limit;
        ...
        if (addonIsInstalled('tele-wpp')) {
            $pricingPlan->telegram_bot_limit  = $telegramAddonIsInstalled ? $request->telegram_bot_limit : 0;
        }
        ...
        $pricingPlan->interactive_message = $request->interactive_message ? Status::YES : Status::NO;
        $pricingPlan->ecommerce_available = $request->ecommerce_available ? Status::YES : Status::NO;
        $pricingPlan->api_available       = $request->api_available ? Status::YES : Status::NO;
        $pricingPlan->save();
